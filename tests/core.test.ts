...
test('full structured pipeline works with a deterministic provider substitute',async()=>{
 ...
 let calls=0;const provider:AIProvider={async json(_instruction,data,schema){calls++;if('reviewFindings' in (data as object))return schema.parse({reviews:(data as {reviewFindings:{id:string}[]}).reviewFindings.map(f=>({id:f.id,verdict:'supported',reason:'Both quotes support the finding'}))});const payload=data as {sources?:Source[];section?:string;before:{evidenceId:string}[];after:{evidenceId:string}[]};if(payload.sources)return schema.parse({entities:payload.sources.filter(s=>s.clause==='3.4').map(s=>({type:'function',name:'Контроль качества',owner:'Департамент анализа данных',sourceId:s.id,quote:s.text}))});return schema.parse({findings:payload.section==='risks'?[]:[{...finding,beforeIds:[payload.before[0].evidenceId],afterIds:[payload.after[0].evidenceId]}]});}};
 const progress:number[]=[];const report=await analyze(b,a,(step)=>progress.push(step),new AbortController().signal,provider);assert.equal(calls,5);assert.equal(report.mode,'live');assert.equal(report.findings[0].status,'confirmed');assert.ok(progress.includes(4));
});
