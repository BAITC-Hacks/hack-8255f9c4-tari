...
const mock=createServer(async(req,res)=>{
 ...
 let result;
 if(data.sources)result={entities:data.sources.filter(s=>s.clause==='2.3.1').slice(0,1).map(s=>({type:'function',name:'Оценка эффективности СВК',owner:'БВА',sourceId:s.id,quote:s.text.slice(0,110)}))};
 else result={findings:[{id:'HTTP-1',kind:'preserved',title:'Оценка СВК сохранена',description:'Проверка HTTP-конвейера с тестовой моделью: цитаты извлечены из обоих PDF.',status:'confirmed',confidence:.9,beforeEvidence:[{sourceId:data.before[0].sourceId,quote:data.before[0].quote}],afterEvidence:[{sourceId:data.after[0].sourceId,quote:data.after[0].quote}]}]};
...
const child=spawn(process.execPath,[require.resolve('next/dist/bin/next'),'start','--hostname','127.0.0.1','--port',String(port)],{stdio:['ignore','pipe','pipe'],env:{...process.env,DEMO_MODE:'false',AI_API_KEY:'test-placeholder',AI_BASE_URL:`http://127.0.0.1:${mock.address().port}/v1`,AI_MODEL:'local-test-model'}});
...
try{
 ...
 const response=await fetch(base+'/api/analyze',{method:'POST',body:data});assert.equal(response.status,200);
 ...
 const invalid=await fetch(base+'/api/analyze',{method:'POST',body:bad});const errorEvents=(await invalid.text()).trim().split('\n').map(JSON.parse);assert.equal(errorEvents.at(-1).error.code,'FORMAT');
 const missing=await fetch(base+'/api/analyze',{method:'POST',body:new FormData()});assert.equal(missing.status,400);
